...
export function RetirementCalculatorTool() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-2xl text-foreground mb-2">Retirement Calculator</h2>
        <p className="text-muted-foreground">
          A tool I built for my own planning — estimate a target retirement age from
          income, expenses, inflation, and investment return assumptions.
        </p>
      </div>
      ...
      <section className="bg-card border border-border rounded-lg p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-display text-xl text-foreground">Your Inputs</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <label className="text-sm text-foreground/80">
            Current Age
            <input type="number" value={currentAge} onChange={(e) => setCurrentAge(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Life Expectancy
            <input type="number" value={lifeExpectancy} onChange={(e) => setLifeExpectancy(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Current Savings (₹)
            <input type="number" value={currentSavings} onChange={(e) => setCurrentSavings(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Monthly Income (₹)
            <input type="number" value={monthlyIncome} onChange={(e) => setMonthlyIncome(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Monthly Expense (₹)
            <input type="number" value={monthlyExpense} onChange={(e) => setMonthlyExpense(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Income Growth (% / year)
            <input type="number" step="0.1" value={incomeGrowthRate * 100} onChange={(e) => setIncomeGrowthRate(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Inflation (% / year)
            <input type="number" step="0.1" value={inflationRate * 100} onChange={(e) => setInflationRate(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Pre-Retirement ROI (% / year)
            <input type="number" step="0.1" value={preRetirementRoi * 100} onChange={(e) => setPreRetirementRoi(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Post-Retirement ROI (% / year)
            <input type="number" step="0.1" value={postRetirementRoi * 100} onChange={(e) => setPostRetirementRoi(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Safe Withdrawal Rate (% / year)
            <input type="number" step="0.1" value={withdrawalRate * 100} onChange={(e) => setWithdrawalRate(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            Retirement Expense Level (% of current)
            <input type="number" step="1" value={retirementExpenseRatio * 100} onChange={(e) => setRetirementExpenseRatio(Number(e.target.value) / 100)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2" />
          </label>
          <label className="text-sm text-foreground/80">
            ROI Profile
            <select value={riskProfile} onChange={(e) => applyRiskProfile(e.target.value as RiskProfile)} className="mt-1 w-full rounded border border-border bg-card px-3 py-2">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2 text-accent-teal">
            <PiggyBank className="w-4 h-4" />
            <span className="text-sm font-medium">Required Corpus</span>
          </div>
          <p className="text-2xl font-bold text-foreground">{formatINR(result.requiredCorpusAtRetirement)}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-foreground">{formatINR(result.annualSavingsNow)}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2 text-accent-sage">
            <Calculator className="w-4 h-4" />
            <span className="text-sm font-medium">Current Savings Rate</span>
          </div>
          <p className="text-2xl font-bold text-foreground">{toPercent(result.savingsRate)}</p>
        </div>
      </section>
